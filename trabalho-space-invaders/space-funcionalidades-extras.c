	...
	for (i = 0; i < mapsLin + 2; i++){
		for(j = 0; j < mapsCol + 2; j++){
			if(EhParaPrintarOMais(jogo.mapa, j, i)){
				...
				if(jogo.danca != 0){
                    if(EhPar(jogo.iter)){
					    fprintf(arquivo, TIRO2);
				    }
				    else{
					    fprintf(arquivo, TIRO1);
				    }
				}
				else {
					fprintf(arquivo, TIRO1);
...
tJogo OcorreuMorte(tJogo jogo, FILE * resumo){
	...
	if(jogo.TiroAtivo == ATIVO){
		for (i = 0; i < jogo.QuantidadeInimigos; i++){
			if(ExisteInimigoAqui(jogo, jogo.ini[i], tiroLin, tiroCol) && !EstaMorto(jogo.ini[i]) && !InimigoCruzouLinha(jogo) && jogo.invensivel != 1){
